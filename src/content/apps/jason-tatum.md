---
slug: jason-tatum
name: Jason Tatum
tagline: Slack-driven coding agent — plan, approve, ship a PR.
live: false
internal: true
stack: [TypeScript, Fastify, BullMQ, Slack Bolt, Railway]
ai_tools: [Cursor Cloud Agents]
live_url: null
repo_url: https://github.com/Trivelta-Vibe-Coders/jason-tatum
hero_image: ../../assets/gallery/jason-tatum/hero.jpeg
portrait: false
built_by: Connor Steeley
shipped: "2026-05-01"
order: 5
blurb: >-
  Orchestrates Cursor Cloud Agents from Slack. Propose with /dunk, agent
  drafts a plan, team approves in thread, agent implements and opens a PR.
  Human-in-the-loop code gen.
---
